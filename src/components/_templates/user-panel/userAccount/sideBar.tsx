...
import useUserInfo from "@/components/_layout/header/headerProfile/hook/useUserInfo";
import { SidebarUserAccount } from "@/static_data/SidebarUserAccount";
    ...
    <>
      <aside className="space-y-10">
        ...
        <div className="flex flex-col">
          {SidebarUserAccount?.map((item) => (
            <Link
              href={item?.link}
              key={item?.id}
              className={`flex cursor-pointer items-center gap-x-2 p-3 ${
                Pathname === item?.link
                  ? "border-l border-solid border-l-primary text-primary"
                  : "border-l border-solid border-l-gray-550 text-gray-550"
              }`}
            >
              <div dangerouslySetInnerHTML={{ __html: item?.icon }} />
              <span className="font-peyda-400 text-base lg:text-lg">
                {item?.name}
              </span>
            </Link>
          ))}
          ...
          >
            ...
            <span className="font-peyda-400 text-base lg:text-lg">خروج</span>
          </div>
        </div>
      </aside>
    </>
